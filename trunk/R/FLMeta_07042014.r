  ...
  if(by == "unit"){
    ...
    for(iTer in 1:nrow(scaleHarvest)){
      ...
      scaleHarvest[iTer,] <- optim(par=stpar,fn=function(mult,iTac,iBiol,iFleet,inBy,iAvail,iScen,iSameUnit){
          ...
          #- equalShare means that all mults are as close together as possible
          # Need to make sure though that mults on same unit are the same, not across units
#            for(iUnit in dimnames(z)$unit)
#              # here we rougly calculate a percentage difference and multiply that with sumres
#              reslst[[iUnit]]   <- sqrt(reslst[[iUnit]]^2) + exp(mean(abs(outer(mult[sameUnit[[iUnit]]],mult[sameUnit[[iUnit]]],"-")))*sqrt(reslst[[iUnit]]^2))
#            res   <- unlist(reslst)
#          }
          print(sum(sqrt(res^2)))
